...
export async function assertLiveNetwork(client, expectedChainId) {
  if (!client || typeof client.getChainId !== 'function') {
    throw new DeploymentUnavailable('The GenLayer client cannot verify its live chain ID.');
  }
  let observed;
  try {
    observed = BigInt(await client.getChainId());
  } catch (error) {
    throw new DeploymentUnavailable(`Could not verify the GenLayer network: ${error?.message ?? error}`);
  }
  if (observed !== BigInt(expectedChainId)) {
    throw new DeploymentUnavailable(`Connected RPC is on chain ${observed}; expected ${expectedChainId}.`);
  }
  return observed;
}
...
export async function loadLiveJobs({ client, address, expectedChainId = 4221, limit = MAX_VISIBLE_JOBS }) {
  ...
  await assertLiveNetwork(client, expectedChainId);
